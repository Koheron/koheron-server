                         ...
                         ListeningChannel<sock_type> *listener)
{
    ...
    listener->kserver->syslog. template print<SysLog::INFO>(
                "Start session id = %u. "
    ...
    if (session->run() < 0)
        listener->kserver->syslog. template print<SysLog::ERROR>(
...
void comm_thread_call(ListeningChannel<sock_type> *listener)
{
    while (!listener->kserver->exit_comm.load()) {
        ...
        if (comm_fd < 0) {
            listener->kserver->syslog. template print<SysLog::CRITICAL>(
