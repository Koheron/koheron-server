...
int WebSocket::authenticate()
{
    ...
    if (pos == std::string::npos) {
        kserver->syslog.print<CRITICAL>("WebSocket: No WebSocket Key");
        return -1;
    }
...
int WebSocket::read_http_packet()
{
    ...
    // Check reception ...
    if (nb_bytes_rcvd < 0) {
        kserver->syslog.print<CRITICAL>("WebSocket: Read error\n");
        return -1;
    }
    ...
    kserver->syslog.print<DEBUG>("[R] HTTP header\n");
...
int WebSocket::read_stream()
{
    ...
    if (unlikely(read_head_err < 0)) {
        kserver->syslog.print<CRITICAL>("WebSocket: Cannot read header\n");
        return -1;
    }
    ...
    if (unlikely(err < 0)) {
        kserver->syslog.print<CRITICAL>("WebSocket: Cannot read payload\n");
        return -1;
    }
...
int WebSocket::check_opcode(unsigned int opcode)
{
    switch (opcode) {
      ...
      case CONNECTION_CLOSE:
        kserver->syslog.print<INFO>("WebSocket: Connection close\n");
        return 1;
      case PING:
        kserver->syslog.print<CRITICAL>("WebSocket: Ping is not suported\n");
        return -1;
      case PONG:
        ...
        kserver->syslog.print<CRITICAL>("WebSocket: Pong is not suported\n");
        return -1;
      default:
        kserver->syslog.print<CRITICAL>("WebSocket: Invalid opcode %u\n", opcode);
        return -1;
    }
...
//    printf("%s\n", read_str);
    ...
    if (unlikely(header.payload_size > WEBSOCK_READ_STR_LEN - 56)) {
        kserver->syslog.print<CRITICAL>("WebSocket: Message too large\n");
        return -1;
    }
...
int WebSocket::read_n_bytes(int64_t bytes, int64_t expected)
{
    ...
    while (expected > 0) {
        ...
        if (bytes_read == 0) {
            kserver->syslog.print<INFO>("WebSocket: Connection closed by client\n");
            connection_closed = true;
            return 1;
        }

        if (unlikely(read_str_len == KSERVER_READ_STR_LEN)) {
            kserver->syslog.print<CRITICAL>("WebSocket: Read buffer overflow\n");
            return -1;
        }
    }
...
int WebSocket::send_request(const unsigned char *bits, long long len)
{
    ...
    kserver->syslog.print<DEBUG>("[S] %i bytes\n", bytes_send);
    return bytes_send;
}
